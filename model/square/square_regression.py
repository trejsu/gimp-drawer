...
def convolutional_network(image):
    ...
    # Map the ARGS.fc1 features to 2 arguments (x,y)
    with tf.name_scope('fc2'):
        W_fc2 = weight_variable([ARGS.fc1, 2])
        b_fc2 = bias_variable([2])

        y_conv = tf.sigmoid(tf.add(tf.matmul(h_fc1_dropout, W_fc2), b_fc2, name="y_conv"))
    ...
    with tf.Session() as sess:
        ...
        for epoch in tqdm.tqdm(range(global_epoch, ARGS.epoch)):
            ...
            for step in tqdm.tqdm(range(num_batches)):
                ...
                mse.append(loss_eval)
                if step == 0:
                    tqdm.tqdm.write('train mse %g' % loss_eval)

            save_model(epoch + 1, saver, sess)
            save_learning_curve(mse)

        mean_test_mse = np.zeros(data.test.batch_n)
        for i in tqdm.tqdm(range(data.test.batch_n)):
            X, Y = data.test.next_batch()
            test_mse = loss.eval(feed_dict={x: X, y: Y, keep_prob: 1.0})
            mean_test_mse[i] = test_mse
            tqdm.tqdm.write('test mse %g' % test_mse)
        print("average test mse = %g" % np.mean(mean_test_mse))
