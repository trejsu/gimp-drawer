...
def main(_):
    ...
    model = Sequential()
    model.add(Conv2D(filters=64, kernel_size=(11, 11), border_mode='full'))
    model.add(BatchNormalization((64, 226, 226)))
    model.add(Activation('relu'))
    model.add(MaxPool2D(poolsize=(3, 3)))

    model.add(Conv2D(filters=128, kernel_size=(7, 7), border_mode='full'))
    model.add(BatchNormalization((128, 115, 115)))
    model.add(Activation('relu'))
    model.add(MaxPool2D(poolsize=(3, 3)))

    model.add(Conv2D(filters=192, kernel_size=(3, 3), border_mode='full'))
    model.add(BatchNormalization((128, 112, 112)))
    model.add(Activation('relu'))
    model.add(MaxPool2D(poolsize=(3, 3)))

    model.add(Conv2D(filters=256, kernel_size=(3, 3), border_mode='full'))
