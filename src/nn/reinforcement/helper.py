    ...
    with open('./Center/frames/log' + str(i) + '.csv', 'w') as myfile:
        ...
        a, v = sess.run([mainQN.Advantage, mainQN.Value], \
                        feed_dict={mainQN.scalarInput: np.vstack(bufferArray[:, 0]) / 255.0,
                                   ...
                                   mainQN.state_in: state_train, mainQN.batch_size: 1})
        wr.writerows(
            zip(bufferArray[:, 1], bufferArray[:, 2], a[:, 0], a[:, 1], a[:, 2], a[:, 3], v[:, 0]))
...
def make_gif(images, fname, duration=2, true_image=False, salience=False, salIMGS=None):
    ...
    if salience == True:
        ...
        mask.write_gif(fname, fps=len(images) / duration,
                       verbose=False)  # clipB.write_gif(fname, fps = len(images) / duration,verbose=False)
    else:
